...
class PlagiarismChecker:
    ...
    def load_database(self):
        """Load previously checked documents from a file."""
        if os.path.exists(self.db_file):
            with open(self.db_file, "r", encoding="utf-8") as file:
                self.database = [line.strip() for line in file.readlines()]
        else:
            self.database = []

    def save_to_database(self, text):
        """Save new text to the file-based database."""
        with open(self.db_file, "a", encoding="utf-8") as file:
            file.write(text + "\n")
    ...
    def add_to_database(self, text):
        cleaned_text = self.clean_text(text)
        if cleaned_text not in self.database:
            self.database.append(cleaned_text)
            self.save_to_database(cleaned_text)
    ...
    def search_online(self, text):
        """Uses SerpAPI (Google Search API) to check if the text is found online."""
        url = "https://serpapi.com/search"
        params = {
            "q": text,
            "api_key": self.serpapi_key,
...
while True:
    ...
    # Add new text to the database after checking
    plagiarism_checker.add_to_database(user_text)
>>>>>>> c45330d0b7749cee57163be5116f241e860a03b7
